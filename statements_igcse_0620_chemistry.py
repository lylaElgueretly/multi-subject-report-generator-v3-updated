...
try:
    ...
    # Maths - All Years (Note: different variable names in your files!)
    from statements_year5_Maths import (
        opening_phrases as opening_5_math,
        attitude_bank as attitude_5_math,
        number_bank as maths_5_math,  # Using number_bank as maths_bank
    ...
    from statements_year7_Maths import (
        ...
        number_and_algebra_bank as maths_7_math,  # Using number_and_algebra_bank
    ...
    from statements_year8_Maths import (
        ...
        maths_bank as maths_8_math,  # This one has maths_bank
        target_bank as target_8_math,
        closer_bank as closer_8_math
    )

    # ESL (IGCSE) - Note: Different structure based on your comment
    # Let me create a fallback for ESL since it has different structure
    try:
        ...
        # Create fallback structure for ESL if file doesn't exist
        opening_esl = ["This term in ESL,"]
        attitude_esl = {75: "has shown good progress in language learning"}
        reading_esl = {75: "has developed reading comprehension skills"}
        writing_esl = {75: "has improved writing skills"}
        speaking_esl = {75: "has participated in speaking activities"}
        listening_esl = {75: "has shown listening comprehension"}
        target_reading_esl = {75: "continue to read regularly"}
        target_write_esl = {75: "practice writing more frequently"}
        target_speak_esl = {75: "participate in speaking practice"}
        target_listen_esl = {75: "listen to English audio regularly"}
        closer_esl = ["Continue the good work."]

    # Chemistry (IGCSE) - Note: Different structure
    try:
        ...
        # Create fallback structure for Chemistry
        opening_chem = ["This term in Chemistry,"]
        attitude_chem = {75: "has engaged well with chemistry concepts"}
        chemistry_chem = {75: "has shown understanding of chemical principles"}
        target_chem = {75: "continue to study key concepts"}
        closer_chem = ["Keep up the good work in Chemistry."]
...
def generate_comment(subject, year, name, gender, att, achieve, target, optional_text=None):
    ...
    if subject == "English":
        ...
        reading_target_sentence = f"For the next term, {p} should {lowercase_first(target_reading_esl[target])}"
        writing_target_sentence = f"Additionally, {p} should {lowercase_first(target_write_esl[target])}"
...
st.caption("Supports: English, Maths, Science, ESL (IGCSE), Chemistry | Years: 5, 7, 8, 10, 11 | Target: 500 characters per comment")
